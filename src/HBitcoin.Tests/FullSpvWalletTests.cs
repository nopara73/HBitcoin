...
namespace HBitcoin.Tests
{
    public class FullSpvWalletTests
    {
        ...
        {
            // load wallet
            Safe safe = Safe.Load(string.Empty, $"../../Wallets/EmptyMain.json");
            Assert.Equal(Network.Main, safe.Network);
        ...
        {
            // load wallet
            Safe safe = Safe.Load(string.Empty, $"../../CommittedWallets/HaveFundsTestNet.json");
            Assert.Equal(Network.TestNet, safe.Network);
