...
namespace NTumbleBit.BouncyCastle.Crypto.Engines
{
	...
	internal class Salsa20Engine
	{
			...
			byte input)
		{
			if(LimitExceeded())
			{
				throw new InvalidOperationException("2^70 byte limit per IV; Change IV");
			}
			...
			int outOff)
		{
			...
			if(LimitExceeded((uint)len))
				throw new InvalidOperationException("2^70 byte limit per IV would be exceeded; Change IV");
